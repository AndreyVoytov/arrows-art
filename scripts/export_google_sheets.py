...
DEFAULT_EQUIPMENT_OUTPUT = CONFIG_DIR / "equipment_prices.json"
DEFAULT_LEVEL_REWARDS_OUTPUT = CONFIG_DIR / "level_rewards.json"
DEFAULT_OBJECTS_RU_OUTPUT = TEXT_TEMP_DIR / "objects_ru.json"
DEFAULT_ROOMS_RU_OUTPUT = TEXT_TEMP_DIR / "rooms_ru.json"
...
def main(argv: list[str] | None = None) -> None:
    ...
    level_rewards = parse_level_rows(level_rows)

    write_json(args.equipment_output, build_equipment_prices(equipment_items))
    write_json(args.level_rewards_output, build_level_rewards(level_rewards))
    write_json(args.objects_ru_output, build_objects_ru(equipment_items))
    write_json(args.rooms_ru_output, build_rooms_ru(rooms))
...
def parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--equipment-output",
        type=Path,
        default=DEFAULT_EQUIPMENT_OUTPUT,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--objects-ru-output",
        type=Path,
        default=DEFAULT_OBJECTS_RU_OUTPUT,
    )
    parser.add_argument(
        "--rooms-ru-output",
        type=Path,
        default=DEFAULT_ROOMS_RU_OUTPUT,
    )
...
def parse_level_rows(rows: list[dict[str, str]]) -> list[dict[str, object]]:
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        levels.append(
            {
                "level": level,
                "complexity": cell(row, "complexity", "compexity"),
...
def build_equipment_prices(items: list[dict[str, object]]) -> dict[str, object]:
...
def build_level_rewards(levels: list[dict[str, object]]) -> dict[str, object]:
...
def build_objects_ru(items: list[dict[str, object]]) -> dict[str, str]:
    ...
    for item in items:
        ...
        if object_name:
            translations[object_key(str(item["objectId"]))] = object_name
...
def update_room_configs(
    ...
    for room_id, room_name in rooms.items():
        ...
        object_index = index_room_objects(room_config)
...
def index_room_objects(room_config: dict[str, object]) -> dict[str, list[dict[str, object]]]:
...
def object_key(object_id: str) -> str:
    return f"{object_id}_key"


def room_key(room_id: str) -> str:
    return f"{room_id}_key"
